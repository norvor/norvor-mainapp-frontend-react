...
const DealCard: React.FC<DealCardProps> = ({ deal }) => (
  <div className="bg-white p-4 rounded-lg shadow mb-4 cursor-pointer hover:shadow-md transition-shadow">
    <h4 className="font-semibold text-sm text-gray-800">{deal.name}</h4>
    <p className="text-xs text-gray-500 mt-1">Value: ${deal.value.toLocaleString()}</p>
...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ stage, deals }) => (
  <div className="flex-shrink-0 w-72 bg-gray-100 rounded-lg p-3">
    <div className="flex justify-between items-center mb-4">
      ...
      <span className="text-xs font-bold text-gray-400 bg-gray-200 rounded-full px-2 py-1">{deals.length}</span>
    </div>
