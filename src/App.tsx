...
const TeamModuleRenderer = () => {
    ...
    const activeTeamMembers = useMemo(() => users, [users]);
    ...
    const activeTeamProjects = useMemo(() => projects, [projects]);
    const activeTeamTickets = useMemo(() => tickets.filter(t => t.teamId === teamId), [tickets, teamId]);

    if (!currentUser) return null;

    switch (module) {
        case 'hub':
            return <TeamHubView teamId={teamId} tickets={activeTeamTickets} teamMembers={activeTeamMembers} projects={activeTeamProjects} />;
        case 'crm':
            return <CrmView viewingUser={currentUser} teamMembers={activeTeamMembers} contacts={contacts} deals={deals} activities={activities} unassignedContacts={unassignedContacts} allTeamContacts={allTeamContacts} allTeamDeals={allTeamDeals} refetchData={() => dispatch(fetchContacts())} />;
        case 'pm':
            return <PmView viewingUser={currentUser} allUsers={users} projects={projects} tasks={tasks} teamMembers={activeTeamMembers} />;
        case 'requests':
            return <RequestsView teamId={teamId!} tickets={activeTeamTickets} allUsers={users} currentUser={currentUser} />;
        case 'docs':
            return <DocsView />;
        case 'hr':
             return <HrView viewingUser={currentUser} allUsers={users} timeOffRequests={[]} directReports={[]} />;
        default:
