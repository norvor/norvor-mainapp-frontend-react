    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-gray-500">Total Sales (Won)</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">${totalSales.toLocaleString()}</p>
            </div>
            <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-gray-500">Overall Conversion Rate</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{conversionRate}%</p>
            </div>
             <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-gray-500">Active Deals</h3>
                <p className="text-3xl font-bold text-gray-900 mt-2">{deals.filter(d => d.stage !== 'Won' && d.stage !== 'Lost').length}</p>
            </div>
...
const SalesForecastingReport: React.FC<{ deals: Deal[] }> = ({ deals }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Projected Revenue by Close Date</h3>
...
const DataExportView: React.FC = () => (
    <div className="bg-white p-6 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-500 mt-1 mb-4">Download all CRM data as CSV files.</p>
...
const ExecutiveCrmView: React.FC<ExecutiveCrmViewProps> = ({ contacts, deals }) => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-4">Executive CRM Control Center</h1>
      <div className="mb-6 border-b border-gray-200">
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          <button onClick={() => setActiveTab('analytics')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'analytics' ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Company-Wide Analytics
          </button>
          <button onClick={() => setActiveTab('forecasting')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'forecasting' ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Sales Forecasting
          </button>
          <button onClick={() => setActiveTab('export')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'export' ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Data Export
          </button>
        </nav>
      </div>
