        ...
        <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
                {contacts.map((contact) => (
                    <tr key={contact.id} onClick={() => onSelectContact(contact)} className="hover:bg-gray-50 cursor-pointer">
    ...
    <div className="bg-white shadow rounded-lg p-6 animate-fade-in">
        ...
        <div className="mt-6">
            ...
            <form className="mt-4 border-t pt-4">
                <textarea rows={3} placeholder="Log new activity..." className="w-full p-2 border border-gray-300 rounded-md text-sm focus:ring-violet-500 focus:border-violet-500"></textarea>
...
const TeamCrmView: React.FC<TeamCrmViewProps> = ({ currentUser, contacts, deals, activities }) => {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'deals') {
      return (
        <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-4">My Deal Kanban</h2>
    ...
    <div>
      <div className="mb-6 border-b border-gray-200">
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          <button onClick={() => { setActiveTab('contacts'); setSelectedContact(null); }} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'contacts' ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            My Contacts
          </button>
          <button onClick={() => setActiveTab('deals')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'deals' ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
